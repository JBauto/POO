...
public class tabu_op implements tabu{

	private HashMap<Integer,List<int[]>> tabu_list;
	private int retries;
	
	public tabu_op (int x){
	...
	public boolean checkTabu(int op,int i,int j){
		...
		if(op_list == null)
			return false; // pode tentar a operacao
		...
		for(Iterator<int[]>it = op_list.iterator(); it.hasNext();){
			...
			if(Arrays.equals(values, vector)){
				if(values[2]!=0){
					...
					return true; // nao tenta a operacao
				}else{
					it.remove();
					return false; // pode tentar a operacao
				}
