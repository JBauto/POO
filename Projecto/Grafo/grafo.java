...
public class grafo {
	...
	public int [][] createGrafo (int [][] Data, int [] r, int type_score){ //type_score==0 --> LL, type_score==1 --> MDL
		int i, j;
		int [][] mat_adj_max = new int[2*r.length][2*r.length]; //inicialisa logo tudo a zero
		...
		if(type_score==0){
			...
			while(true){ // enquanto existir um vizinho com score mais alto que mat_adj_max ou outro criterio de paragem
				...
				for(i=0;i<2*r.length;i++){
					for (j=r.length;j<2*r.length;j++){ // todas as adiçoes possiveis
				...
				for(i=0;i<2*r.length;i++){
					for (j=r.length;j<2*r.length;j++){ // todas as subtracçoes possiveis 
